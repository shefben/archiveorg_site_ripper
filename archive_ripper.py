...
def download_page(archive_url: str, output_dir: str, concurrency: int, savename: str = None):
    ...
    if savename:
        ...
        final_name = root + ext
        final_dir = os.path.dirname(local_page)
        final_path = os.path.join(final_dir, os.path.basename(final_name))

        os.makedirs(final_dir, exist_ok=True)
        os.replace(local_page, final_path)
        local_page = final_path

    log(f"Page saved to {local_page}")
    return local_page


def run_ripper(url: str, output_dir: str, concurrency: int = 1, savename: str = None, reset: bool = False):
    if reset:
        path = os.path.join(output_dir, '.downloaded.txt')
        if os.path.exists(path):
            os.remove(path)
    conc = min(concurrency, MAX_CONCURRENCY)
    page = download_page(url, output_dir, conc, savename)
    log(f"Saved page to {page}")
    return page


# -----------------------
# GUI + console focusing
# -----------------------

def focus_console_window():
    """Try to bring the console window to the foreground (Windows only)."""
    if os.name != 'nt':
        return
    try:
        import ctypes
        ...
        # If this fails, we just silently ignore it. Console output still exists.
        pass
...
def launch_gui():
    ...
    class RipperWorker(QtCore.QThread):
        ...
        def run(self):
            try:
                if not self.save_path:
                    ...
                    output_dir = os.path.dirname(self.save_path) or 'output'
                    savename = os.path.basename(self.save_path)
                page = run_ripper(self.url, output_dir, concurrency=1, savename=savename, reset=False)
                self.finished_ok.emit(page)
            except Exception as e:
    ...
    class MainWindow(QtWidgets.QWidget):
        def __init__(self):
            super().__init__()
            self.setWindowTitle("Archive.org Ripper")
            self.worker = None
            self._build_ui()
        ...
        def _build_ui(self):
            ...
            form.addRow("Archive URL:", self.url_edit)

            save_layout = QtWidgets.QHBoxLayout()
            self.save_edit = QtWidgets.QLineEdit()
            self.save_edit.setPlaceholderText("Choose file to save as (e.g. output/index.html)")
            ...
            self.go_btn.clicked.connect(self.start_rip)
        ...
        def start_rip(self):
            ...
            # bring console to foreground so logs are visible
            focus_console_window()

            self.go_btn.setEnabled(False)
            self.worker = RipperWorker(url, self.save_edit.text().strip(), self)
            self.worker.finished_ok.connect(self.on_done)
            self.worker.error.connect(self.on_error)
            self.worker.finished.connect(self.on_finished_thread)
            self.worker.start()

        def on_done(self, page_path: str):
            QtWidgets.QMessageBox.information(
                self,
                "Done",
                f"Page saved to:\n{page_path}\n\nCheck the console window for full log output.",
            )
...
def cli_main():
    ...
    parser.add_argument(
        '-s',
        '--savename',
        help=('Filename to write the main page to. If you omit an extension I '
              'will keep the one from the original page (usually ".html").'),
    )

    args = parser.parse_args()
    run_ripper(args.url, args.output, args.concurrency, args.savename, args.reset)


def main():
    # If no arguments are provided, launch the Qt5 GUI.
    if len(sys.argv) == 1:
        launch_gui()
    else:
        cli_main()
